...
### Cross-discipline tuning rewrite (2026-07-24, uncommitted)

The research gathered after the RX-7 session is now implemented across all modes. `tuning.ts` uses cold pressure profiles by surface/compound, drivetrain-specific drag settings, power-peak/limiter-aware gearing without fixed loose-surface speeds, low-camber/high-caster alignment, compliant loose suspension, front-biased road aero, neutral road braking and explicit FWD/RWD/AWD differential tables. Drift engine scoring now penalizes maximum flywheel/cam/boost tiers. Telemetry passes the built drivetrain and diagnoses limiter, driven-axle slip, temperatures and suspension travel. Symptom advice directions were audited and corrected. `PROGRESS.md` and `docs/tuning-engine-design.md` contain the details. Full check is green at 110 tests and the production PWA build passes. Do not push without explicit permission.
